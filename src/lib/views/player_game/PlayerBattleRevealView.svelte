...
<div class="w-full h-[100dvh] max-h-[100dvh] min-h-0 flex flex-col justify-between p-3.5 sm:p-4 max-w-md mx-auto safe-top safe-bottom select-none font-sans overflow-hidden box-border">
    ...
    {#if battle?.prompt}
        <div class="bg-neutral-950/90 border border-neutral-800 rounded-xl p-2.5 mb-2.5 text-center shadow-md flex-shrink-0">
            ...
            <p class="text-xs sm:text-sm font-semibold text-slate-200 leading-snug line-clamp-2">
                {battle.prompt}
            </p>
        </div>
    {/if}
    ...
    {#if isCompetitor}
        ...
        <main class="flex-1 flex flex-col justify-between p-3.5 sm:p-4 bg-neutral-950/90 border-2 {isWinner ? 'border-amber-400 shadow-[0_0_25px_rgba(251,191,36,0.5)]' : 'border-slate-600'} rounded-2xl overflow-y-auto">
            
            <div>
                <!-- Status Banner -->
                <div class="w-full py-1 px-2 rounded-lg text-center font-display font-black text-xs sm:text-sm uppercase tracking-wider mb-2 {
                    isWinner 
                ...
                <div class="text-center my-1.5">
                    <span class="text-[9px] font-display font-bold text-slate-400 uppercase tracking-widest block mb-1">
                        {$t.yourAnswer || 'Your Punchline'}
                    </span>
                    <div class="bg-black/80 border border-neutral-700 rounded-xl p-2 text-xs sm:text-sm font-bold text-white leading-relaxed">
                        {renderAnswer(battle?.answers?.[player?.id])}
                    </div>
                </div>

                <!-- Points Breakdown -->
                <div class="flex flex-col items-center justify-center my-2 text-center">
                    ...
                    {#if myScoreBreakdown.rainbowBonus > 0}
                        <div class="flex justify-center my-1">
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-display font-black tracking-wide bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-cyan-500/20 border border-pink-400 text-pink-300 shadow-[0_0_10px_rgba(236,72,153,0.4)] animate-pulse">
                                {$t.rainbowBadge || '🌈 Rainbow Bonus'} (+{myScoreBreakdown.rainbowBonus})
                            </span>
                        </div>
                    {/if}

                    <div class="flex flex-wrap items-center justify-center gap-1.5 mt-1 text-[10px] font-mono">
                        <span class="px-2 py-0.5 rounded bg-neutral-800 border border-neutral-700 text-neutral-300">
                        ...
                        {#if myScoreBreakdown.winBonus > 0}
                            <span class="px-2 py-0.5 rounded bg-amber-950 border border-amber-500 text-amber-300">
                                🏆 {$t.winBonus || 'Win Bonus'} (+{myScoreBreakdown.winBonus})
                            </span>
                        {/if}
                        {#if myScoreBreakdown.sweepBonus > 0}
                            <span class="px-2 py-0.5 rounded bg-purple-950 border border-purple-500 text-purple-300">
