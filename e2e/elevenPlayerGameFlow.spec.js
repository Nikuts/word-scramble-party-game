...
test.describe('11-Player Hybrid (1 vs 1 and 1 vs 1 vs 1) Game Flow', () => {
    test('plays 11-player game with hybrid 6 trios (1 vs 1 vs 1) and 2 duos (1 vs 1) across 8 battles per round', async ({ browser }) => {
        ...
        // 2. Setup 11 Players with unique avatars on Mobile Contexts
        const playerConfigs = [
            { name: 'Alice', avatar: '👽' },
            { name: 'Bob', avatar: '🦊' },
            { name: 'Charlie', avatar: '🤖' },
        ...
        await expect(
            hostPage.locator('.host-lobby-container, .host-game-container, .panel-arcade, .scoreboard, [data-testid="game-id"], h1, h2').first()
        ).toBeVisible({ timeout: 25000 });
